...
import com.example.dilangpinoy.objects.AnalogyLevel;
...
public class AnalogyActivity extends MainStageActivity {
    ...
    private List<AnalogyLevel> analogyLevel;
    ...
    public void getStageLevels(String stageFile) {
        analogyLevel = readFileWithAnswers(mContext, stageFile);
        Collections.shuffle(analogyLevel);
    }
    ...
    public void initLevel(int level) {
        ...
        for (int i = 0; i < choices.length; i++) {
            ...
            switch (options[i]) {
                case 0:
                    choices[i].setText(analogyLevel.get(level).getCorrectAnswer());
                    break;
                case 1:
                    choices[i].setText(analogyLevel.get(level).getWrongAnswer1());
                    break;
                case 2:
                    choices[i].setText(analogyLevel.get(level).getWrongAnswer2());
                    break;
            }
    ...
    public List<AnalogyLevel> readFileWithAnswers(Context context, String stageFile) {
        List<AnalogyLevel> phrasesList = new ArrayList<AnalogyLevel>();
        String json = null;
        try {
        ...
        try {
            ...
            for (int i = 0; i < pairs.length(); i++) {
                phrasesList.add(gson.fromJson(pairs.getString(i), AnalogyLevel.class));
