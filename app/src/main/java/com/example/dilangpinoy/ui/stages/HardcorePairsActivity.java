...
import com.example.dilangpinoy.objects.HardPairLevel;
...
public class HardcorePairsActivity extends MainStageActivity {
    ...
    private List<HardPairLevel> hardpairLevels;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hardcorepairs);
    ...
    public void getStageLevels(String stageFile) {
        hardpairLevels = readFileWithPairs(mContext, stageFile);
        Collections.shuffle(hardpairLevels);
    }
    ...
    public void initLevel(int level) {
        ...
        for (int i = 0; i < choices.length; i++) {
            ...
            if (i < 4)
                ...
                choices[i].setText(hardpairLevels.get(level).getPairs().get(leftOptions[i]).getOption1());
            else
                choices[i].setText(hardpairLevels.get(level).getPairs().get(rightOptions[i - 4]).getOption2());
    ...
    public List<HardPairLevel> readFileWithPairs(Context context, String stageFile) {
        List<HardPairLevel> pairsList = new ArrayList<HardPairLevel>();
        String json = null;
        try {
        ...
        try {
            ...
            for (int i = 0; i < pairs.length(); i++) {
                pairsList.add(gson.fromJson(pairs.getString(i), HardPairLevel.class));
